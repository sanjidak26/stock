...
export async function runAiQuery(question, shopId) {
  ...
  // If the query is not matched by any template above, verify if it is relevant
  // before falling back to the inventory snapshot.
  const keywords = [
    'stock', 'inventory', 'medicine', 'medicines', 'drug', 'drugs', 'tablet', 'tablets',
    'capsule', 'capsules', 'syrup', 'quantity', 'qty', 'expire', 'expiry', 'expired',
    'expiring', 'date', 'revenue', 'sale', 'sales', 'sell', 'selling', 'sold', 'billing',
    'bill', 'bills', 'invoice', 'price', 'cost', 'loss', 'profit', 'value', 'dealer',
    'dealers', 'supplier', 'suppliers', 'supply', 'supplied', 'distributor', 'distributors',
    'shop', 'pharmacy', 'store', 'hi', 'hello', 'hey', 'greetings', 'help', 'thanks', 'thank you',
    'who are you', 'who you', 'what can you do', 'features'
  ];

  let isRelevant = keywords.some((kw) => q.includes(kw));

  if (!isRelevant) {
    try {
      const databaseHandle = await db();
      const medicines = await databaseHandle.collection('medicines').find({ shop_id: shop }).toArray();
      for (const med of medicines) {
        const medName = String(med.name || '').toLowerCase();
        if (medName && q.includes(medName)) {
          isRelevant = true;
          break;
        }
      }
    } catch (err) {
      console.error('Error checking medicine names for AI query:', err);
    }
  }
  ...
  if (!isRelevant) {
    return {
      outOfScope: true,
      message: "I won't be able to answer irrelevant questions. Please ask questions related to StockEasy Management."
    };
  }
